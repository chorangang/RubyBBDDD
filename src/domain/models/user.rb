...
class User
    ...
    def initialize(hash)
        ...
        @id         = hash['id']
        @name       = Name.new(hash['name'])
        @email      = Email.new(hash['email'])
        @password   = Password.new(hash['password'])
        @created_at = hash['created_at'] || Time.now.strftime("%Y-%m-%d %H:%M:%S")
        @updated_at = hash['updated_at'] || Time.now.strftime("%Y-%m-%d %H:%M:%S")
    end
